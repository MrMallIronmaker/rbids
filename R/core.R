  ...
  public = list(
    ...
    initialize = function(root, readonly = TRUE) {
      if (!is.character(root) || length(root) != 1) {
        stop("`root` must be a single character string.")
      }
      if (!is.logical(readonly) || length(readonly) != 1) {
        stop("`readonly` must be a single logical value.")
      }
      ...
      if (!dir.exists(root)) {
        if (readonly) {
          ...
          warning(sprintf("Directory `%s` does not exist. Creating directory...", root))
    ...
    print = function() {
      ...
      subjects <- paste(paste(head(sort(unique(self$index$subject)), 5), collapse = ", "), "...")
